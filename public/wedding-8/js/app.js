...
$(document).ready(function() {
	...
	$('#wishes_form').submit(function() {
		submitWish();
		return false;
	});
});
function submitRSVP() {
	$('.btn-attending').html('Sending...');
	$.ajax({
        url : 'http://eyoore.com/api/post_rsvp.php',
        method : 'POST',
        data : { 'name' : $('#rsvp-name').val(), 'email' : $('#rsvp-email').val(), 'numberOfGuest' : $('#rsvp-number_of_guest').val(), 'ww_id' : $('#ww_id').val() },
        success : function(response){
            ...
            $('.rsvp-please').hide();
            $('.rsvp-desc').fadeIn();
            $('.rsvp-desc-thanks').html('Thank you <b class="rsvp-name">'+response.data.name+'</b>, your RSVP has been submitted to us.');
            $('.rsvp-title').html('Yes, You\'re Attending!');
            return false;
        }
    });
    return false;
...
function loadWishes() {
      $.ajax({
            url : 'http://eyoore.com/api/get_wishes.php?ww_id=3',
            method : 'GET',
            success : function(data){
...
function loadRsvp() {
  $.ajax({
        url : 'http://eyoore.com/api/get_rsvp.php?ww_id=3',
        method : 'GET',
        success : function(data){
            //$('.table-rsvp').html(data);
			var trHTML = '';
            $.each(data, function(i, item) {
				trHTML += '<tr><td>' + (i+1) + '</td><td>' + item.name + '</td><td>' + item.email + '</td><td>' + item.number_of_guest + '</td></tr>';
		    });
